...
<div class="am-tabs am-margin" data-am-tabs>
  <ul class="am-nav am-nav-tabs"><!-- am-tabs-nav  -->
      <li id="tabli0" ><a href="__URL__?state=0">未审核</a></li>
      <li id="tabli1" ><a href="__URL__?state=1">已审核</a></li>
      <li id="tabli2" ><a href="__URL__?state=2">已封禁</a></li>
      ...
      <div class="am-u-sm-12">
        ...
        <!-- <form class="am-form" method="POST" action="__URL__/check"> -->
          <table class="am-table am-table-striped am-table-hover table-main">
            <thead>
              <tr>
                <th class="table-id">UID</th><th class="table-title">昵称</th><th class="table-author">标题</th><th class="table-author">组</th><th class="table-author">评语</th><th class="table-author">操作</th>
              </tr>
          </thead>
          <tbody>
            
            <?php if(is_array($videoList)): foreach($videoList as $key=>$video): ?><tr>
              <td>UID<?php echo ($video["user_id"]); ?></td>
              <td><a href="#"><?php echo ($video["nicheng"]); ?></a></td>
              <td><a href="__URL__/checkVideo?vid=MAD<?php echo ($video["video_id"]); ?>"><?php echo ($video["title"]); ?></a></td>
              <td><?php echo ($video["group_name"]); ?></td>
              <td class="comment"><?php echo ($video["comment"]); ?></td>
              <td>
                <div class="am-btn-toolbar">
                  <div class="am-btn-group am-btn-group-xs">
                     
                    <a href="__URL__/addRelease?id=<?php echo ($release["id"]); ?>" class="am-btn am-btn-default am-btn-xs am-text-danger"><span class="am-icon-trash-o"></span> 审核</a>
                    <a href="__URL__/delRelease?id=<?php echo ($release["id"]); ?>" class="am-btn am-btn-default am-btn-xs"><span class="am-icon-trash-o"></span> 封禁</a>
                  </div>
                </div>
              </td>
...
<script type="text/javascript">
  $(function(){
      ...
      if($('#state').val() == ''){
        $('.am-nav-tabs li').eq(0).addClass('am-active');
      }else{
        $('#tabli'+$('#state').val()).addClass('am-active');
      }
