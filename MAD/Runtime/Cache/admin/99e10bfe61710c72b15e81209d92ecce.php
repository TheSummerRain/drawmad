        ...
        <form class="am-form" method="POST" action="__URL__/purview"> 
          ...
          <tbody>
            ...
            <?php if(is_array($userList)): foreach($userList as $key=>$user): ?><tr> 
              <td>UID<?php echo ($user["id"]); ?></td>
              <td><a href="__APP__/MyZone?userid=<?php echo ($user["id"]); ?>" target="_blank"><?php echo ($user["nicheng"]); ?></a>
              <td><?php echo ($user["email"]); ?></td>
              <td><?php echo ($user["video_num"]); ?></td>
              <td><?php echo ($user["name_type"]); ?></td>
              <td><?php echo (date('Y年m月d日', $user["reg_time"])); ?></td>
              <td>
                <div class="am-btn-toolbar">
                  <div class="am-btn-group am-btn-group-xs">

                  <?php if($user["user_state"] == 1): ?><a  href="#" class="am-btn am-btn-default am-btn-xs am-text-danger"><span class="am-icon-trash-o"></span> 封禁</a>
                  <?php else: ?>
                    <a href="#" class="am-btn am-btn-default am-btn-xs am-text-secondary"><span class="am-icon-pencil-square-o"></span>解封</a><?php endif; ?>
                    <a href="__URL__/purview?userId=<?php echo ($user["id"]); ?>" class="am-btn am-btn-default am-btn-xs am-text-secondary"><span class="am-icon-pencil-square-o"></span>权限</a>
