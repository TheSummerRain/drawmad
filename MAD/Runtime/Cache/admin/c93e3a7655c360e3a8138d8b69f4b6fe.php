...
<div class="am-tabs am-margin" data-am-tabs>
        ...
        <form class="am-form" method="POST" action="__URL__/updatePurview">
          <div class="am-g am-margin-top">
            <div id="roleName" class="am-u-sm-2 am-text-right">
              用户Id
            </div>
            <div class="am-u-sm-4">
              <input id="roleGroupIdV" type="text" disabled="disabled" class="am-input-sm" name="id" value="UID<?php echo ($userInfo["id"]); ?>">
            </div>
          ...
          <div class="am-g am-margin-top">
            ...
            <div class="am-u-sm-4">
              <input id="roleNameV" type="text" disabled="disabled" class="am-input-sm" name="nicheng" value="<?php echo ($userInfo["nicheng"]); ?>">
            </div>
            <div class="am-u-sm-6"></div>
          </div>
          ...
          <div class="am-g am-margin-top-sm">
            ...
            <input type="hidden" name="userType" value="ut<?php echo ($userInfo["user_type"]); ?>">
            <input type="hidden" name="roleGroupIds" value="<?php echo ($userInfo["role_group_id"]); ?>" id="roleGroupIds">
            <!-- <div class="am-u-sm-4 current_purview_id" style="display:inline-block">
              <?php if(is_array($roleGroupList)): foreach($roleGroupList as $key=>$role): ?><div style="display:inline-block">
                  <input type="checkbox" id="PPID<?php echo ($role["id"]); ?>" value="<?php echo ($role["id"]); ?>" name="role[]" class="type_checkbox" >
                  <label class="check" style=" font-weight:normal"><?php echo ($role["name"]); ?></label>
                </div><?php endforeach; endif; ?>
            </div> -->

            <div class="am-u-sm-4 current_purview_id" style="display:inline-block">
              <?php if(is_array($userTypeList)): foreach($userTypeList as $key=>$usert): ?><div style="display:inline-block; margin-right:10px;">
                  <input type="radio" id="ut<?php echo ($usert["id"]); ?>" value="<?php echo ($usert["id"]); ?>" name="userTypeReal" class="type_radio" >
                  <label class="check" for="ut<?php echo ($usert["id"]); ?>" style=" font-weight:normal"><?php echo ($usert["type"]); ?></label>
                </div><?php endforeach; endif; ?>
            </div>
...
<script type="text/javascript">
  $(document).ready(function(){
      ...
      var roidGroupIdsV = $('#roleGroupIds').val().split(',');
      for(var i=0; i< roidGroupIdsV.length; i++){
          $('#PPID'+roidGroupIdsV[i]).prop('checked', true);
      }

      $('#'+$('input[name=userType]').val()).attr("checked",'checked');
